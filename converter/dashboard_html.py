...
def render_report(report):
    ...
    for key, source, title, description, options in (
            ...
            ('daily-errors', 'daily', 'Daily errors', 'Failed attempts · compare with the activity above', [('errors', 'Errors')]),
            ('monthly-errors', 'monthly', 'Monthly errors', 'Striped month is partial',
             [('errors', 'Errors'), ('errors_per_day', 'Errors per calendar day')]),
...
const metrics = {attempts:['Attempts by outcome','attempts'],errors:['Errors','errors'],unique_users:['Approximate users','users'],attempts_per_day:['Attempts per calendar day','attempts/day'],errors_per_day:['Errors per calendar day','errors/day'],error_rate:['Failure rate','%'],rates:['Failure rate','%'],latency:['Duration','seconds'],stages:['Failures by stage','errors']};
...
function draw(section, rows) {
  ...
  const barMode=['attempts','errors','attempts_per_day','errors_per_day'].includes(key);
  ...
  const max=unit==='%'?100:ceiling(maximum,['attempts','errors','users'].includes(unit));
